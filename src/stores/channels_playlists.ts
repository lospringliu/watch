import { IChannel } from "../types"
export const CHANNELS = [ 
    { id: 'UC7Ky7FjJBI7ojx2Yqz2pkNQ', name: '萨沙', title: 'SHA SA 萨莎 Саша' },
    { id: 'UCH2BtkEp1kHY7o9TCIhYaFA', name: '萨沙环球', title: '萨莎-环球频道-shasa-Саша' },
    { id: 'UCwNGgFvBpxtU8JagZLzztzQ', name: '岩论', title: 'Yan Talk' },
    { id: 'UCkZ9vSvx7lg9FvZifpbH2FA', name: '老兵', title: '民族阵线-老兵莱昂' },
    { id: 'UCE05tYKEsEk7Qmhwg5pqcKw', name: '杨峰', title: '杨风時評' },
    { id: 'UCd6umYVQpBZ9CIyCwe8Kg7w', name: '诚阅', title: '诚阅 ChengYue' },
    { id: 'UCXRoo1Gp89SCbhdR_xEIp0w', name: '北美崔哥', title: 'BrotherSway北美崔哥' },
    { id: 'UCnrxxRlv2ZSSW4ApuEy8C0w', name: '谦秋论', title: '谦秋论官方' },
    { id: 'UCnUprfZhHRzfcLHdLS-f6aw', name: '司马南', title: '司马南频道' },
    { id: 'UCC22zTYPmf9p20E_abhp-uw', name: '残月', title: '残月两家之言' },
    { id: 'UCr_F4Y9iboUKlg_ZPm4jkVQ', name: '老梁', title: '老梁' },
    { id: 'UCJncdiH3BQUBgCroBmhsUhQ', name: '观察网', title: '观察者网' },
    { id: 'UCXkOTZJ743JgVhJWmNV8F3Q', name: '寒国人', title: '寒國人' },
    { id: 'UCSgGqt-30oXBq0-n0K-t4Nw', name: '科技袁人', title: "Yuan's Multiverse科技袁人" },
    { id: 'UCEjqTrvJdLG7Eo0KP1AzYJw', name: '大佬时空', title: '大佬时空' },
    { id: 'UCUBhobCkTLhgfUNRAgHSYmw', name: '科学声音', title: '科学声音' },
    { id: 'UCD_gy8DWV_DhjJ-bQXF5dGQ', name: '磊哥', title: '磊哥聊政经' },
    { id: 'UCI-ECZ-pbvFL2fLtskq2GPA', name: '老伍', title: '老伍和他的朋友们' }
  ] as IChannel[]
export const PLAYLISTS = [
    { id: 'PLBsA1M_J2ICIvMjsj_U7lIdeq-E8AW-bM', name: '杨世光', title: '楊世光在金錢爆' },
    { id: 'UURdpxiOm4HrwGI24Kv6MBKQ', name: 'David郑经纬', title: 'David郑经纬' },
    { id: 'UURByPS00RZsAUe2DTCoHuFQ', name: '好奇大叔', title: '好奇大叔 Uncle Curious' },
    { id: 'UUdXqCN_HtF_RjlsHzDSnJIQ', name: '德国知事', title: '德国知事GermanCheese' },
    { id: 'UUoCHlYM3srHBmg21mK1-JPg', name: '老楊到處說', title: '老楊到處說' },
    { id: 'UU2IpIQXiLrMV3EAMjfKbcUw', name: '雪石', title: '雪石看世界' },
    { id: 'UU-8fdTrDRgiJhSq3wRsaF-g', name: '寒梅', title: '寒梅视角MEI HAN' },
    { id: 'UUOo6xtjQu3LuRzMTfFkGrSA', name: '迷彩虎', title: '迷彩虎' },
    { id: 'UU5uh3zVGmvyQoks_LxBJ-5Q', name: '郑国成', title: '中国青年郑国成' },
    { id: 'UUnyPxKrdrXDjlBBucRAIuuA', name: '圆脸波士顿', title: '波士顿自干五-圆脸传媒' },
    { id: 'UUgo_-fjJxnLwwwq5dSY72rg', name: '自话总裁', title: '自说自话的总裁' }
  ] as IChannel[]